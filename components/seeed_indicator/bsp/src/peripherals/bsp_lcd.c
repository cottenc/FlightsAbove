...
esp_err_t bsp_lcd_init(void)
{
...
#if CONFIG_LCD_AVOID_TEAR
            // DMA reads from internal SRAM while the CPU refills each bounce
            // buffer from PSRAM, preventing RGB frame drift under contention.
            .bounce_buffer_size_px = brd->LCD_WIDTH * 20,
#endif
            .flags.fb_in_psram = 1,
#if CONFIG_LCD_AVOID_TEAR
            .flags.double_fb = 1,
    ...
    if (LCD_IFACE_RGB == brd->LCD_IFACE) {
        ...
        memset(lcd_buf1, 0, frame_bytes);
        Cache_WriteBack_Addr((uint32_t)(uintptr_t)lcd_buf0, frame_bytes);
        Cache_WriteBack_Addr((uint32_t)(uintptr_t)lcd_buf1, frame_bytes);
    }
#endif
    ...
    if (GPIO_NUM_NC != brd->GPIO_LCD_BL) {
        ...
        // RGB scanout starts before the application has composed its first
        // complete frame. Keep it hidden until the UI explicitly reveals it.
        gpio_set_level(brd->GPIO_LCD_BL,
                       LCD_IFACE_RGB == brd->LCD_IFACE
                           ? !brd->GPIO_LCD_BL_ON
                           : brd->GPIO_LCD_BL_ON);
    }

    // Drawing an RGB clear one scanline at a time repeatedly swaps/disturbs
    // the live DMA framebuffer. The two RGB buffers were initialized above.
    if (LCD_IFACE_RGB != brd->LCD_IFACE) {
        screen_clear(0x00ff);
    }
    return ret_val;
