...
static const board_res_desc_t g_board_lcd_evb_res = {
    ...
    // GX screen (FT6336U) has its touch origin at the bottom-right corner, so
    // both axes need inversion to match the display's top-left origin.
    .TOUCH_PANEL_INVERSE_X =    (CONFIG_SENSECAP_INDICATOR_SCREEN_GX ? 1 : 0),
    .TOUCH_PANEL_INVERSE_Y =    (CONFIG_SENSECAP_INDICATOR_SCREEN_GX ? 1 : 0),
...
esp_err_t bsp_board_sensecap_indicator_init(void)
{
    if (g_board_lcd_evb_res.FUNC_IO_EXPANDER_EN) {
        ...
        // FT6336U needs time to boot after reset before it accepts I2C traffic.
        Delay(200);
    }
