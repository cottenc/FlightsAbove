...
esp_err_t ft5x06_init(void)
{
    ...
    // Responsive without letting panel noise become a permanent held touch.
    ft5x06_write_byte(FT5x06_ID_G_THGROUP, 45);

    // Valid touching peak detect threshold
    ft5x06_write_byte(FT5x06_ID_G_THPEAK, 55);
    ...
    ft5x06_write_byte(FT5x06_ID_G_THDIFF, 18);

    // Delay to enter 'Monitor' status (s)
    ft5x06_write_byte(FT5x06_ID_G_TIME_ENTER_MONITOR, 10);

    // Period of 'Active' status (ms)
    ft5x06_write_byte(FT5x06_ID_G_PERIODACTIVE, 8);

    // Timer to enter 'idle' when in 'Monitor' (ms)
    ft5x06_write_byte(FT5x06_ID_G_PERIODMONITOR, 20);

    uint8_t touch_points = 0;
    return ft5x06_read_byte(FT5x06_TOUCH_POINTS, &touch_points);
}

esp_err_t ft5x06_read_pos(uint8_t *touch_points_num, uint16_t *x, uint16_t *y)
{
    static uint8_t data[5];
    esp_err_t ret_val = ft5x06_read_bytes(FT5x06_TOUCH_POINTS, sizeof(data), data);
    if (ret_val != ESP_OK) {
        *touch_points_num = 0;
        return ret_val;
    }
